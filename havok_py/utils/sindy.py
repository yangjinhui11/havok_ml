...
def sindy_setup(Xin, poly_order, use_sine, t, method='derivative', dt_max=None):
    if method == 'integral':
        ...
        print("Sindy for continue system method:",method)
        LHS = differentiate(Xin, t, dt_max=dt_max)
        if np.isscalar(t) or (dt_max is None):
            RHS, labels = pool_data(Xin[:,1:-1], poly_order, use_sine)
            #RHS, labels = pool_data(Xin[:,2:-2], poly_order, use_sine)
        else:
            time_gaps = np.where((t[1:] - t[:-1] > dt_max) | (t[1:] - t[:-1] < 0))[0]
            if time_gaps.size == 0:
                RHS, labels = pool_data(Xin[:,1:-1], poly_order, use_sine)
        ...
        print("Sindy for discrete system")
        RHS, labels = pool_data(Xin[:,:-1], poly_order, use_sine)
        LHS = Xin[:,1:]
        labels = np.ones(len(Xin[0,:]))
        return RHS, LHS, labels
...
class SINDy:
    ...
    def fit(self, Xin, poly_order, t=None, Xprime=None, coefficient_threshold=.01, dt_max=None):
        RHS, LHS, labels = sindy_setup(Xin, poly_order, self.use_sine, t, method=self.differentiation_method,
                                       dt_max=dt_max)
        self.labels = labels
        normTheta = (np.sqrt(np.sum(RHS**2, axis=1)))
        #for i in range(RHS.shape[0]):
        #    RHS[i,:] /=normTheta[i]
        
        n,T = LHS.shape
        #Xi = np.zeros((RHS.shape[0],n))
        Xi = la.lstsq(RHS.T,LHS.T)[0]
        small_inds = (np.abs(Xi) < coefficient_threshold)
        for k in range(10):
            #small_inds = (np.abs(Xi) < coefficient_threshold)
            for i in range(n):
                small_inds[:,i] = np.abs(Xi[:,i]) < coefficient_threshold*(i+1)
            Xi[small_inds] = 0
            for i in range(n):
                #small_inds = np.abs(Xi[:,i]< coefficient_threshold)
                #Xi[small_inds,i] = 0
                big_inds = ~small_inds[:,i]
                if np.where(big_inds)[0].size == 0:
                    continue
                Xi[big_inds,i] = la.lstsq(RHS[big_inds].T, LHS[i])[0]
        self.RHS = RHS
        self.LHS = LHS
        self.poly_order = poly_order
        self.Xi = Xi
        self.error = (np.mean((LHS - np.dot(Xi.T,RHS))**2,axis=1))
